...
void handle_read (char* buf, int blockno) {
  ...
  /*
    Called after interrupt_service_routine
    dequeses pending_read and writes to buf
  **/
...
void run (int num_blocks) {
  ...
  /*
  Need to:
    - call init_pending_read and place in a queue? (Enqueue handled by schedule_read)
    - call interrupt_service_routine for each read competion
  **/
  for (int blockno = 0; blockno < num_blocks; blockno++) {
    // TODO
    // Initialized pr and registers handler
    // SOMETHING WRONG HERE
    //printf("buf = %d, blockno = %d, sizeof(buf) = %lu\n",*((int*) buf),blockno, sizeof(*buf));
    //pr[blockno] = malloc(sizeof(struct pending_read));
...
int main (int argc, char** argv) {
  ...
  uthread_init (1);
  disk_start   (interrupt_service_routine);   // register with isr and sends interrupt
  queue_init   (&prq);
